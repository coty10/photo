...
class FAScrollView: UIScrollView{
    ...
    func zoom() {
        print(zoomScale)
        if (zoomScale <= 1.0) { setZoomScale(zoomScaleWithNoWhiteSpaces(), animated: true); print("nowhite")}
        else{ setZoomScale(minimumZoomScale, animated: true); print("white") }
        updateLayout()
    }
    func tapToZoom() {
        if (zoomScale <= 1.5) { setZoomScale(maximumZoomScale, animated: true)}
        else{ setZoomScale(minimumZoomScale, animated: true); print("white") }
        updateLayout()
    }
